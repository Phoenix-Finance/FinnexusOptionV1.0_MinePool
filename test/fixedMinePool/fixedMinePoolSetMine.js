...
contract('fixedMinePool', function (accounts){
    it('fixedMinePool Locked stake FPTB function', async function (){
        ...
        await minePool.setMineCoinInfo(CFNXA.address,2000000,2);
        let nowId = await minePool.getCurrentPeriodID();
        console.log("current period :",nowId.toNumber());
        let netWorth = await minePool.getTokenNetWorth(CFNXA.address,nowId);
        let settleTime = await minePool.getMinerBalance(accounts[0],CFNXA.address);
        assert.equal(netWorth.toNumber(),0,"getTokenNetWorth Error");
        await minePool.stakeFPTA(100000);;
        await minePool.stakeFPTB(100000,2);
        netWorth = await minePool.getTokenNetWorth(CFNXA.address,nowId);
        settleTime = await minePool.getMinerBalance(accounts[0],CFNXA.address);
        ...
        assert.equal(contractBalance.toNumber(),1000000000100000,"getUserFPTABalance Error");
        result = await minePool.getDistributionCal(accounts[0]);
        assert.equal(result[0].toString(),"100000000","getDistributionCal Error");
        ...
        await minePool.redeemMinerCoin(CFNXA.address,11999999);
        netWorth = await minePool.getTokenNetWorth(CFNXA.address,nowId);
        settleTime = await minePool.getMinerBalance(accounts[0],CFNXA.address);
        console.log("getTokenNetWorth : ",netWorth.toString(),settleTime.toString());
    });
    it('fixedMinePool Locked stake FPTB function unStakeA', async function (){
        ...
        assert.equal(fptB.toNumber(),100000,"getUserFPTBBalance Error");
        contractBalance = await CFNXA.balanceOf(minePool.address);
        assert.equal(contractBalance.toNumber(),1000000000200000,"contractBalance Error");
        result = await minePool.getDistributionCal(accounts[0]);
