...
contract('integratedStake', function (accounts){
    ...
    it('integratedStake stake USDC and FNX ,locked', async function () {
        let contracts = await deployMinePool();
        let curPeriod = await contracts[0].getCurrentPeriodID();
        let expiraiton = await contracts[0].getPeriodFinishTime(curPeriod.toNumber()+2)
        let inputs = {
            usdAddr : [USDC.address],
            amountA : [10000000000],
            fnxAddr : [FNX.address],
        ...
        let checkOut =["10000000000000000000000","28000000000000000000000",curPeriod.toNumber()+2,expiraiton.toNumber()]
        await testStakeContract(contracts,inputs,checkOut)
    })
    it('integratedStake stake USDC,USDT and FNX ,locked', async function () {
        ...
        let checkOut =["30000000000000000000000","28000000000000000000000",curPeriod.toNumber()+2,expiraiton.toNumber()]
        await testStakeContract(contracts,inputs,checkOut)
    })
    ...
    it('integratedStake stake USDC then FNX，locked', async function () {
        ...
        checkOut =["10000000000000000000000","28000000000000000000000",curPeriod.toNumber()+2,expiraiton.toNumber()]
        await testStakeContract(contracts,inputs,checkOut)
    })
    ...
    it('integratedStake stake USDC,USDT and FNX ,add locked', async function () {
        let contracts = await deployMinePool();
        let curPeriod = await contracts[0].getCurrentPeriodID();
        let expiraiton = await contracts[0].getPeriodFinishTime(curPeriod.toNumber()+1)
        let inputs = {
            usdAddr : [USDC.address,USDT.address],
            amountA : [10000000000,20000000000],
            fnxAddr : [FNX.address],
        ...
        let checkOut =["30000000000000000000000","28000000000000000000000",curPeriod.toNumber()+1,expiraiton.toNumber()]
        await testStakeContract(contracts,inputs,checkOut)
        expiraiton = await contracts[0].getPeriodFinishTime(curPeriod.toNumber()+2)
        inputs.lockedPeriod = 3
        checkOut =["60000000000000000000000","56000000000000000000000",curPeriod.toNumber()+2,expiraiton.toNumber()]
        await testStakeContract(contracts,inputs,checkOut)
    })
    ...
    async function testStakeContract (contracts,inputs,checkOut) {
        ...
        let lockTime = await contracts[0].getAverageLockedTime();
        console.log("average lockTime : ",lockTime.toNumber())
    }
